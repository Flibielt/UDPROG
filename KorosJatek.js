function setup() {
  ...
  var ido = 0; //mennyi maradéknak kell lennie, amikor lejár az idő
...
function draw() { 
 ...
 fill(0); // a betűk így feketék maradnak
 ...
 if((kajax-x)*(kajax-x)+(kajay-y)*(kajay-y)-r*r<0){
   ido = frameCount % elkapasi_ido - 1; //maradékos osztás az elkapási idővel
   // -1, mert amúgy rögtön lejárna az idő
   if(ido == -1){
     //a -1 kivételkezelése
    ido = elkapasi_ido + ido; 
   }
   //így mindig elkapási időnyi mp lesz 
   eredmeny += 1;
   elkapta = frameCount; //az idő amikor elkapta
