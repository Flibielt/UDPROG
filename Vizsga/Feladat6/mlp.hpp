...
public:
  ...
  {
  	int iS = 360000;
  	double* copiedImage = new double[iS];
  	for(int i{0}; i<iS; ++i) {
		copiedImage[i] = image[i];
  	}
    units[0] = copiedImage;
    ...
    for ( int i {1}; i < n_layers; ++i )
      ...
      double returnValue = sigmoid ( units[n_layers - 1][0] );
      delete copiedImage;
      return returnValue;
  }

  double operator() ( double* image, double** originalImage )
  {
  	int iS = 360000;
  	double* copiedImage = new double[iS];
  	for(int i{0}; i<iS; ++i) {
  ...
  double* getImage( double* image, double* result )
  {
    units[0] = image;
    ...
    for ( int i {1}; i < n_layers; ++i )
      ...
      *result = sigmoid ( units[n_layers - 1][0] );
      return image;
